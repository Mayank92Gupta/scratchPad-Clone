...
const MidArea = () => {
  ...
  const handleElementClick = (actionType, value) => {
    if (actionType === "move") {
      ...
      moveSpriteLeft(value);
      dispatch(QueueAction("ENQUEUE", `move_left ${value}`));
    }
    else if (actionType === "moveUp") {
      moveSpriteVertically(value, "up");
      dispatch(QueueAction("ENQUEUE", `move_up ${value}`));
    }
    else if (actionType === "moveDown") {
      moveSpriteVertically(value, "down");
      dispatch(QueueAction("ENQUEUE", `move_down ${value}`));
    }
    ...
    <div className="relative h-full w-full">
      ...
      >
        {droppedElements.map((elem, elemIndex) => (
          ...
          >
            <span>{elem.actionType === "move" && "Move Right"}</span>
            <span>{elem.actionType === "moveLeft" && "Move Left"}</span>
            <span>{elem.actionType === "moveUp" && "MoveUp"}</span>
            <span>{elem.actionType === "moveDown" && "Move Down"}</span>
            ...
            {elem.actionType === "moveLeft" && <span>steps</span>}
            {elem.actionType === "moveUp" && <span>steps</span>}
            {elem.actionType === "moveDown" && <span>steps</span>}
